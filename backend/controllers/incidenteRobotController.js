...
// Actualizar registro
export const updateIncidentRobot = async (req, res) => {
  const { id } = req.params;
  const robot_incident = req.body;

  try {
    const [affectedRows] = await RobotIncident.update(robot_incident, {
      where: { id },
    });
    ...
    const updated = await RobotIncident.findByPk(id);
